...
public class ServerGridOverlay extends MapOverlay {
    ...
    protected void refresh(Coord cut) {
        ...
        this.location = Location.xlate(new Coord3f((float)(this.ul.x * sgridsz.x), (float)(-this.ul.y * sgridsz.y), 0.0F));


        Coord c = new Coord();
        FloatBuffer vbuf = getBackBuffer();
        vbuf.rewind();
        for (c.y = (int) this.ul.y; c.y < this.ul.y + size.y; c.y++)
            for (c.x = (int) this.ul.x; c.x < this.ul.x + size.x; c.x++) {
